...
namespace EscapeWpf.Model
{
    public class EscapeModel
    {
        ...
        public int loadSavedGame()
        {
            ...
            {
                MessageBox.Show("Extrem fatal brutal error: Failed to load game.", "Error", MessageBoxButton.OK);
                return -1;
            }
        }
        ...
        {
            if (x == _player.X && y == _player.Y) return "Blue";
            if (x == _mines[0].X && y == _mines[0].Y) return "Red";
            if (x == _mines[1].X && y == _mines[1].Y) return "Red";
            if (x == _enemies[0].X && y == _enemies[0].Y) return "Black";
            if (x == _enemies[1].X && y == _enemies[1].Y) return "Black";
